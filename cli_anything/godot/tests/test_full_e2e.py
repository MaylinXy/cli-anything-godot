...
class TestE2EDemoGamePipeline:
    ...
    def test_14_complete_game_assembly(self):
        ...
        data = json.loads(result.output)
        # New export shape (per-preset): preset label is "all-runnable" and
        # individual results live under "exported", each carrying a returncode.
        assert data["preset"] == "all-runnable"
        assert "exported" in data
        assert all("returncode" in r for r in data["exported"])
